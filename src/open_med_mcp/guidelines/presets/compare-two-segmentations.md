...
3. `render_view(img, masks=[pred, ref], mode="contour")` - two contour colours make disagreement
   obvious; add `layout="montage"` for the extent of the disagreement along the stack.
4. Summarise: where the disagreement is (poles, boundary, extra components), its size in mL and
